...
public class PropertyController {
  ...
  @GetMapping("/search")
  public ResponseEntity<ApiResponse> searchProperty(@RequestParam String city, @RequestParam int guestCount, @RequestParam int roomCount, String sortField, String sortDirection, int page, int size) {
    try {
      List<Property> properties = propertyService.searchProperty(city, guestCount, roomCount, sortField, sortDirection, page, size);
      if (properties.isEmpty()) {
        return ResponseEntity.status(NOT_FOUND).body(new ApiResponse("Property not found!", null));
      }
      List<PropertyDto> convertedProperties = propertyService.getConvertedProperty(properties);
